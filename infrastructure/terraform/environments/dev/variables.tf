variable "environment" {
  ...
  validation {
    ...
    error_message = "This stack is pinned to environment=dev."
  }
}
...
variable "acm_certificate_arn" {
  type        = string
  description = "ACM certificate ARN for ALB HTTPS (eu-central-1). Leave empty until DNS exists; ALB + CloudFront use HTTP to origin in that case."
  default     = ""
}
