variable "environment" {
  ...
  validation {
    ...
    error_message = "This stack is pinned to environment=staging."
  }
}
...
variable "acm_certificate_arn" {
  type        = string
  description = "ACM certificate ARN for ALB HTTPS. Empty = HTTP-only ALB / HTTP CloudFront origin (bootstrap only)."
  default     = ""
}
...
variable "documents_upload_cors_allowed_origins" {
  type        = list(string)
  description = "Tarayıcıdan S3 presigned PUT için CORS. Örn. [\"https://web.staging.example.com\"]. Boş bırakılırsa yükleme CORS ile başarısız olur."
  default     = []
}
